...
exports.ProjectDetectorImpl = exports.ProjectDetectorError = void 0;
const vscode = require("vscode");
const fs = require("fs");
const path = require("path");
class ProjectDetectorError extends Error {
...
exports.ProjectDetectorError = ProjectDetectorError;
class ProjectDetectorImpl {
    /**
     ...
     */
    scanForAiProject(dirPath, depth) {
     ...
     */
    collectAiProjects(dirPath, depth, projects) {
        if (depth > ProjectDetectorImpl.MAX_SCAN_DEPTH) {
            return;
        }
        try {
    ...
    /**
     * Gets project structure for the first detected .ai-project
     * If multiple projects exist, returns the first one found
     */
    getProjectStructure() {
        const aiProjects = this.findAllAiProjects();
        if (aiProjects.length === 0) {
            throw new ProjectDetectorError('No .ai-project directory found in workspace', 'NO_PROJECT_FOUND');
        }
     ...
     */
    getProjectStructureForPath(aiProjectDir) {
        return {
            configPath: path.join(aiProjectDir, 'config.json'),
            requirementsPath: path.join(aiProjectDir, 'requirements.md'),
            designPath: path.join(aiProjectDir, 'design.md'),
     ...
     */
    async repairProjectStructure(aiProjectDir) {
        const errors = [];
        const warnings = [];
        try {
     ...
     */
    validateProjectStructure(aiProjectDir) {
        try {
...
exports.ProjectDetectorImpl = ProjectDetectorImpl;
ProjectDetectorImpl.AI_PROJECT_DIR = '.ai-project';
ProjectDetectorImpl.KIRO_DIR = '.kiro';
ProjectDetectorImpl.REQUIRED_FILES = ['config.json', 'requirements.md', 'design.md', 'tasks.md', 'progress.json'];
ProjectDetectorImpl.MAX_SCAN_DEPTH = 3;
