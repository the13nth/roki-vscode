...
class ProgressTrackerImpl {
     ...
     */
    async sendProgressToDashboard(projectId, progressData) {
        try {
     ...
     */
    async analyzeCommitForTaskCompletion(commit) {
        try {
            const structure = this.projectDetector.getProjectStructure();
            const tasks = this.parseTasksFromFile(structure.tasksPath);
     ...
     */
    async analyzeCommitFiles(commit, tasks) {
     ...
     */
    calculateFileCompletionConfidence(filePath, commit) {
        let confidence = 0;
        const fileName = path.basename(filePath).toLowerCase();
     ...
     */
    findTasksRelatedToFile(tasks, filePath) {
     ...
     */
    async detectTaskCompletionFromGit(filePath) {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder)
                return false;
            ...
            const relativePath = vscode.workspace.asRelativePath(filePath);
            const { stdout } = await this.execAsync(`git log --oneline -n 5 --follow -- "${relativePath}"`, { cwd: workspaceFolder.uri.fsPath });
            if (!stdout.trim())
                return false;
     ...
     */
    async analyzeWorkspaceProgress() {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder)
            return;
        try {
            // Get git statistics
            const { stdout: diffStat } = await this.execAsync('git diff --stat HEAD~1 HEAD', { cwd: workspaceFolder.uri.fsPath });
            if (diffStat.trim()) {
     ...
     */
    async suggestProgressReview() {
        const choice = await vscode.window.showInformationMessage('📊 Significant code changes detected. Would you like to review task progress?', 'Review Progress', 'Update Tasks', 'Dismiss');
        switch (choice) {
