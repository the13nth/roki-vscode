...
import { ProjectLoader } from './lib/projectLoader';
import { SyncService } from './lib/syncService';
import { ProjectStateUpdater } from './lib/projectStateUpdater';
import { TaskDocumentProvider } from './lib/taskDocumentProvider';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        (global as any).aiProjectManagerContext = context;
    ...
    const projectStateUpdater = new ProjectStateUpdater();
    ...
    const forceRefreshCommand = vscode.commands.registerCommand('aiProjectManager.forceRefresh', async () => {
        try {
            ...
            vscode.window.showInformationMessage(
                `Sidebar refreshed. Auth: ${isAuthenticated ? 'Yes' : 'No'}, User: ${user?.email || 'None'}`
            );
        } catch (error) {
    ...
    const loadProjectCommand = vscode.commands.registerCommand('aiProjectManager.loadProject', async (projectId?: string) => {
        try {
            const authService = AuthService.getInstance();
            
            if (!authService.isAuthenticated()) {
                const choice = await vscode.window.showInformationMessage(
                    'You need to login first to load projects from the cloud.',
                    'Login',
                    'Cancel'
                );
    ...
    const loadProjectRequirementsCommand = vscode.commands.registerCommand('aiProjectManager.loadProjectRequirements', async (projectId: string) => {
        try {
            const authService = AuthService.getInstance();
    ...
    const selectProjectCommand = vscode.commands.registerCommand('aiProjectManager.selectProject', async (projectId: string) => {
        try {
    ...
    const openCloudDocumentCommand = vscode.commands.registerCommand('aiProjectManager.openCloudDocument', async (projectId: string, documentType: string) => {
        try {
            const authService = AuthService.getInstance();
            ...
            const response = await authService.makeAuthenticatedRequest(
                `${dashboardUrl}/api/vscode/projects/${projectId}/documents/${documentType}`
            );
    ...
    const openLocalDocumentCommand = vscode.commands.registerCommand('aiProjectManager.openLocalDocument', async (filePath: string) => {
        try {
    ...
    const listProjectsCommand = vscode.commands.registerCommand('aiProjectManager.listProjects', async () => {
        try {
            ...
            const projectList = projects.map(p => 
                `• ${p.name} - ${p.description} (${p.progress}% complete)`
            ).join('\n');
    ...
    const openAuthSettingsCommand = vscode.commands.registerCommand('aiProjectManager.openAuthSettings', async () => {
        try {
            ...
            // Show instructions
            vscode.window.showInformationMessage(
                '🌐 Browser opened! Copy the token from the browser and click "Enter Token" in the sidebar to continue.',
                'OK'
            );
        } catch (error) {
    ...
    const checkTokenCommand = vscode.commands.registerCommand('aiProjectManager.checkToken', async () => {
        try {
            ...
            if (!token) {
                const choice = await vscode.window.showWarningMessage(
                    'No authentication token found. Would you like to set one up?',
                    'Set Up Token',
                    'Cancel'
                );
            ...
            vscode.window.showErrorMessage(
                ...
                if (choice === 'Try Again') {
                    ...
                    const config = vscode.workspace.getConfiguration('aiProjectManager');
                    config.update('authToken', '', vscode.ConfigurationTarget.Global);
                    AuthService.getInstance().clearUserSettings();
                    sidebarProvider.refresh();
                }
            });
        }
    ...
    const updateProjectStateCommand = vscode.commands.registerCommand('aiProjectManager.updateProjectState', async () => {
        try {
            ...
            const completedTasks = update.tasks.filter((t: { status: string; }) => t.status === 'done').length;
    ...
    const openTaskEditorCommand = vscode.commands.registerCommand('aiProjectManager.openTaskEditor', async () => {
        try {
            ...
            const tasksFiles = await vscode.workspace.findFiles(
                '**/tasks.md',
                '**/node_modules/**'
            );
            ...
            if (tasksFiles.length === 0) {
                ...
                const createChoice = await vscode.window.showInformationMessage(
                    'No tasks.md file found. Would you like to create one?',
                    'Create tasks.md',
                    'Cancel'
                );
...
`);
    ...
    context.subscriptions.push(
        ...
        updateProjectUserInfoCommand,
        loginCommand,
        logoutCommand,
        loadProjectCommand,
        listProjectsCommand,
        loadProjectRequirementsCommand,
        selectProjectCommand,
        openCloudDocumentCommand,
        openLocalDocumentCommand,
        forceSyncCommand,
        checkCloudChangesCommand,
        showSyncStatusCommand,
        openAuthSettingsCommand,
        enterTokenCommand,
        checkTokenCommand,
        updateProjectStateCommand,
        refreshCloudProgressCommand,
 ...
 */
async function updateProjectWithUserInfo(workspacePath: string, user: any): Promise<void> {
    const kiroSpecsPath = path.join(workspacePath, '.kiro', 'specs', 'ai-project-manager');
    const configPath = path.join(kiroSpecsPath, 'config.json');
