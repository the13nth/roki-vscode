...
export class ProjectLoader {
  ...
  async listUserProjects(): Promise<CloudProject[]> {
  ...
  async loadProjectToWorkspace(projectId: string): Promise<void> {
    ...
    const response = await this.authService.makeAuthenticatedRequest(
      `${dashboardUrl}/api/projects/${projectId}/export`
    );
    ...
    const projectFiles: ProjectFiles = projectData.files;
  ...
  private async createProjectStructure(
    ...
    fs.writeFileSync(
      path.join(kiroSpecsPath, 'progress.json'), 
      JSON.stringify(initialProgress, null, 2)
    );
    fs.writeFileSync(
      path.join(aiProjectPath, 'progress.json'), 
      JSON.stringify(initialProgress, null, 2)
    );
  }

  async showProjectPicker(): Promise<string | undefined> {
    try {
      const projects = await this.listUserProjects();
      
      if (projects.length === 0) {
        vscode.window.showInformationMessage(
          'No projects found. Create a project first using the web dashboard.',
          'Open Dashboard'
        ).then(choice => {
  ...
  async getProjectDetails(projectId: string): Promise<{ name: string; description: string; requirements?: string } | null> {
    try {
      if (!this.authService.isAuthenticated()) {
        throw new Error('Not authenticated');
      }
